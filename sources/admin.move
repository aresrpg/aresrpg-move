...
/// Promote an address to an admin temporarily
entry fun admin_promote_address(admin: &AdminCap, recipient: address, ctx: &mut TxContext) {
...
#[test_only]
public(package) fun test_promote_address(recipient: address, ctx: &mut TxContext) {
  let cap = AdminCap {
    id: object::new(ctx),
    epoch: ctx.epoch(),
  };
